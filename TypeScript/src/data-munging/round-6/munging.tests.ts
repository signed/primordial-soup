...
function minimumIn<T extends { pivot: number }>(values: () => T[], sentinel: T) {
...

{
  ...
  const dayWithSmallestSpread = () => {
    const values = () => extractDataLines(readFileSync(pathToData).toString(), lineFilter).map(lineToValue)
